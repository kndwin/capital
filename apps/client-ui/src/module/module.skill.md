...
license: Apache-2.0
compatibility: Designed for Claude Code; assumes @tanstack/virtual-file-routes and the @capital/server-core RPC contract.
metadata:
...
- Don't share types by reaching into `../<other-feature>/...`. Import
  from `@capital/server-core/rpc` (the contract is the shared type surface).
