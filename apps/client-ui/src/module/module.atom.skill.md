...
license: Apache-2.0
compatibility: Designed for Claude Code; assumes effect (effect/unstable/reactivity, effect/unstable/rpc, effect/unstable/http) and @capital/server-core's RPC contract.
metadata:
...
- Build against the shared `ApiGroup` from `@capital/server-core/rpc`.
...
`@capital/client-api/rpc` — don't re-declare `RpcClient.layerProtocolHttp` per
module.
